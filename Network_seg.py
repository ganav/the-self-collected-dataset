...
def generator():
    ...
    model = Conv2D(filters = 512, kernel_size = 3, strides = 2, padding = "same")(inp)
    model = Conv2D(filters = 512, kernel_size = 3, strides = 2, padding = "same")(model)

    model = res_res_res_block(model)

    model = Conv2D(filters = 128, kernel_size = 3, strides = 1, padding = "same")(model)
    model = UpSampling2D()(model)
    model = UpSampling2D()(model)
    model = Conv2D(filters = 3, kernel_size = 3, strides = 1, padding = "same",name='last_conv')(model)
